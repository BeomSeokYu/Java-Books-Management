...
public class BookManagement {
	...
	public void borrow(int bookid) {
		...
		List<RentVO> a = renDao.selectAllRent(rvo.getId());
		if (a.size() >= 5) {
	...
	// 대여목록 (COM_003)
	public void borrowList(String id) {
		...
		if (list == null || list.size() < 1) {
			...
			System.out.println("대여번호\t   반납일\t\t반납마감일\t\t저자명\t\t\t      제목");
			for (int i = 0; i < list.size(); i++) {
				RentVO rvo = list.get(i);
				System.out.println("  " + rvo.getRentId() + "\t" + 
	...
	// 대여정보 상세보기 (COM_004)
	public void borrowDetail(RentVO rvo, String id) {
	...
	// 반납기한 연장
	public void borrowExtend(RentVO rvo) {
		rvo.setId(Pub.id);
		System.out.println(Constant.EL_S);
	...
	public void reserve(int bookId) {
		...
		RentVO rvo = renDao.selectId(revo.getId());
		System.out.println(Constant.EL_S);
		System.out.println(Constant.EL_S);
		if (rvo != null) {
	...
	public void reserStatus(String id) {
		...
		if (list == null || list.size() < 1) {
			...
			System.out.println("예약번호\t도서번호\t예약일\t\t대여상태\t     저자\t   제목");
			for (ReserVO r : list) {
				System.out.print(r.getReserId() + "\t");
				System.out.print(r.getBookid() + "\t");
				System.out.print(r.getReserDate() + "\t");
				if (r.getRentAvail().equalsIgnoreCase("Y")) {
	...
	public void prpsBookList(String id) {
		...
		if(prpsList == null || prpsList.size() < 1) {
			...
			System.out.println("번호 | 저자명 | 출판사 | 책제목");
			for(AppVO avo : prpsList) {
				System.out.println(avo.getAppId() + " | " + avo.getAuthor() + " | " + avo.getPublisher() + " | " + avo.getBookName());
			}
